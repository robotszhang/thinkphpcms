...
var select_arr = ["#s_province","#s_city","#s_country"];//三个select的name
var opt0 = ["北京市","北京市","东城区"];//初始值

/*
 *更新option值
 *index 0-更新省市县   1-更新市县    2-更新县
 *value 更新第一级的默认值
 */
function refresh_select(index,array){
	//取需要更新的option内容
	var str="0";
	for(i=0;i<index;i++){
		var selected = $(select_arr[i]).prop('selectedIndex');
		selected = typeof(selected) == 'undefined' ? 0 : selected;
		str += "_"+selected;
	};
	//需更新的容器：s[index]
	var option_con = dsy.Items[str];
	$(select_arr[index]).html('');
	for(i=0;i<option_con.length;i++){
		if(array[index] == option_con[i]){
			$(select_arr[index]).append("<option selected='selected' value='"+option_con[i]+"'>"+option_con[i]+"</option>");
		}else{
			$(select_arr[index]).append("<option value='"+option_con[i]+"'>"+option_con[i]+"</option>");
		}
	}
	if(index<select_arr.length-1){

		index++;
		refresh_select(index,array);
	}
}

function _init_area(){  //初始化函数

	refresh_select(0,opt0);

	for(var j=0;j<select_arr.length-1;j++){
		$(select_arr[j]).on('change',new Function('refresh_select('+(j+1)+',["'+opt0[0]+'","'+opt0[1]+'","'+opt0[2]+'"])'));
	}
}
