...
function kDialog(param,callback){
	...
	$dom.appendTo("body");
	$dom.css({left:w-$dom.outerWidth()/2,top:h-$dom.outerHeight()/2+s});
	$dom.draggable({
		handle: ".head",
		containment:"body"
	});
...
function kText(param,callback){
	...
	$dom.appendTo("body");
	var dom_left=w-$dom.outerWidth()/2;
	var dom_top=h-$dom.outerHeight()/2+s;
...
function kWin(param,callback){
	...
	$dom.find(".g_win_container").mCustomScrollbar();
	...
	this.contain = function(str){
		$dom.find("#mCSB_1_container").append(str);
	};
	this.dom = function(){
		return $dom.find("#mCSB_1_container");
	};
	function remove(){
		if($content){
